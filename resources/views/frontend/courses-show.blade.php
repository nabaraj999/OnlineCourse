        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div class="lg:col-span-5">
                <div class="bg-white rounded-3xl shadow-2xl border border-gray-100 p-10">
                    ...
                    <!-- Pending Warning -->
                    @if($hasPending ?? false)
                        <div class="mb-8 p-6 bg-yellow-100 border-2 border-yellow-400 rounded-2xl text-yellow-800 text-center font-bold text-lg">
                    ...
                    @if($course->active_discount)
                        <div class="text-center">
                            ...
                            <span class="inline-block mt-4 px-8 py-3 bg-red-600 text-white rounded-full font-bold text-lg">
                                {{ $course->discount_percentage_active }}% OFF!
                            </span>
                        </div>
                    @else
                        <p class="text-7xl font-black text-primary text-center">NPR {{ number_format($course->discounted_price_npr) }}</p>
                    @endif
                    ...
                    @if($course->available_seats > 0 && !$hasPending ?? true)
                        <button onclick="openEnrollmentModal()" class="w-full bg-gradient-to-r from-primary to-indigo-800 hover:from-indigo-900 hover:to-primary text-white font-black text-3xl py-10 rounded-2xl shadow-2xl transform hover:scale-105 transition-all duration-300">
                            ENROLL NOW
                        </button>
                    @else
...
<div id="enrollmentModal" class="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-3xl max-w-4xl w-full max-h-[95vh] overflow-y-auto">
        ...
        <form id="enrollForm" action="{{ route('courses.enroll.store') }}" method="POST" enctype="multipart/form-data" class="p-10 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="full_name" required class="w-full px-6 py-4 rounded-xl border focus:border-primary focus:ring-4 focus:ring-primary/20 transition text-lg">
                    <p class="error-text text-red-600 text-sm mt-1"></p>
                </div>
                <div>
                    <label class="block font-bold mb-2">Email <span class="text-red-500">*</span></label>
                    <input type="email" name="email" required class="w-full px-6 py-4 rounded-xl border focus:border-primary focus:ring-4 focus:ring-primary/20 transition text-lg">
                    <p class="error-text text-red-600 text-sm mt-1"></p>
                </div>
                <div>
                    <label class="block font-bold mb-2">Phone <span class="text-red-500">*</span></label>
                    <input type="tel" name="phone" required placeholder="+977 ..." class="w-full px-6 py-4 rounded-xl border focus:border-primary focus:ring-4 focus:ring-primary/20 transition text-lg">
                    <p class="error-text text-red-600 text-sm mt-1"></p>
                </div>
                <div>
                    <label class="block font-bold mb-2">Transaction Reference <span class="text-red-500">*</span></label>
                    <input type="text" name="reference_code" required class="w-full px-6 py-4 rounded-xl border focus:border-primary focus:ring-4 focus:ring-primary/20 transition text-lg">
                    <p class="error-text text-red-600 text-sm mt-1"></p>
                </div>
            </div>
            ...
            <div>
                ...
                <select name="payment_method_id" required onchange="showPaymentDetails(this)" class="w-full px-6 py-4 rounded-xl border focus:border-secondary focus:ring-4 focus:ring-secondary/20 transition text-lg">
                    <option value="">-- Select Payment Method --</option>
                    @foreach($payment_methods as $method)
                        <option value="{{ $method->id }}" data-qr="{{ $method->qr_code ? Storage::url($method->qr_code) : '' }}"
                                data-number="{{ $method->account_number }}" data-holder="{{ $method->account_holder }}">
            ...
            <div id="paymentDetails" class="hidden bg-gradient-to-r from-primary/5 to-indigo-50 border-2 border-dashed border-primary/30 rounded-2xl p-8 text-center">
                ...
                <img id="qrCode" src="" alt="QR" class="mx-auto w-56 h-56 rounded-xl shadow-xl mb-6 hidden">
            ...
            <div>
                ...
                <label for="screenshot" class="block border-4 border-dashed border-gray-300 rounded-2xl p-12 text-center hover:border-primary cursor-pointer bg-gray-50">
            ...
            <button type="submit" id="submitBtn" class="w-full bg-gradient-to-r from-primary to-indigo-800 hover:from-secondary hover:to-orange-600 text-white font-black text-2xl py-8 rounded-2xl shadow-2xl transform hover:scale-105 transition-all duration-300">
                SUBMIT ENROLLMENT
            </button>
        </form>
        ...
        <div id="successMessage" class="hidden p-16 text-center bg-gradient-to-b from-green-50 to-white">
            ...
            <code id="generatedPassword" class="bg-gray-100 px-12 py-8 rounded-2xl text-4xl font-black text-secondary tracking-widest block mt-4"></code>
...
<script>
    ...
    function showPaymentDetails(select) {
        ...
        if (opt.dataset.qr) { qr.src = opt.dataset.qr; qr.classList.remove('hidden'); }
